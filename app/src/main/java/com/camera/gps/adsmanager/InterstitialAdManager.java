...
public class InterstitialAdManager {
    ...
    /**
     * Preload one interstitial placement without showing it. The fallback publisher is
     * attempted when the primary publisher fails so callers can safely wait for a
    ...
    public void preloadPlacement(Activity activity, String adsName, Runnable onLoaded, AdCallback onFailed) {
        try {
            ...
            if (!remoteConfig.isShowAds() || adsData == null || !adsData.isEnableAds()) {
                if (onFailed != null) onFailed.onFailure("Placement disabled or missing");
                return;
            }

            String primaryPublisher = adsData.getPublishers().toLowerCase();
            String fallbackPublisher = adsData.getAdFailed() == null
                    ? ""
                    : adsData.getAdFailed().toLowerCase();

            preloadWithFallback(activity, primaryPublisher, fallbackPublisher, onLoaded, onFailed);
        } catch (Exception e) {
            if (onFailed != null) onFailed.onFailure("System error: " + e.getMessage());
        }
    }

    private void preloadWithFallback(Activity activity, String primaryPublisher, String fallbackPublisher,
                                     Runnable onLoaded, AdCallback onFailed) {
        if (isAdReadyForPublisher(primaryPublisher)) {
        ...
        loadAdWithPublisher(activity, primaryPublisher,
                ...
                primaryError -> {
                    if (!fallbackPublisher.isEmpty()
                            && !fallbackPublisher.equals(primaryPublisher)) {
                        if (isAdReadyForPublisher(fallbackPublisher)) {
                            if (onLoaded != null) onLoaded.run();
                        } else {
                            loadAdWithPublisher(activity, fallbackPublisher,
                                    onLoaded,
                                    fallbackError -> {
                                        if (onFailed != null) {
                                            onFailed.onFailure("Primary: " + primaryError
                                                    + ", fallback: " + fallbackError);
                                        }
                                    });
                        }
                    } else if (onFailed != null) {
                        onFailed.onFailure(primaryError);
                    }
                });
    }
...
//    }
    ...
    public void loadAndShowInterstitialAd(Activity activity, String adsName, Runnable onAdClosed, AdCallback onAdFailed) {
        try {
            ...
            // Navigation ads are cached-only: never make a click wait for an ad load.
            if (isAdReadyForPublisher(primaryPublisher)) {
                LogUtils.logI(TAG, "PRIMARY READY - SHOWING PRIMARY");
                ...
                showAdWithPublisher(activity, primaryPublisher, adsName, onAdClosed, onAdFailed, primaryShowError -> {
                    ...
                    if (!fallbackPublisher.isEmpty() &&
                        ...
                        showAdWithPublisher(activity, fallbackPublisher, adsName, onAdClosed, onAdFailed, fallbackShowError -> {
                            LogUtils.logE(TAG, "BOTH SHOW FAILED - PROCEED WITHOUT AD");
                            proceedWithoutAdAndPreload(activity, primaryPublisher, fallbackPublisher, onAdClosed);
                        });
                    } else {
                        LogUtils.logI(TAG, "FALLBACK NOT READY - PROCEED WITHOUT WAITING");
                        proceedWithoutAdAndPreload(activity, primaryPublisher, fallbackPublisher, onAdClosed);
                    }
                });
            } else if (!fallbackPublisher.isEmpty() &&
                ...
                LogUtils.logI(TAG, "NO CACHED AD - PROCEED WITHOUT WAITING");
                proceedWithoutAdAndPreload(activity, primaryPublisher, fallbackPublisher, onAdClosed);
            }
    ...
    private void proceedWithoutAdAndPreload(Activity activity, String primaryPublisher,
                                            String fallbackPublisher, Runnable onAdClosed) {
        preloadPublisher(activity, primaryPublisher);
        if (!fallbackPublisher.isEmpty() && !fallbackPublisher.equals(primaryPublisher)) {
            preloadPublisher(activity, fallbackPublisher);
        }
        if (onAdClosed != null) onAdClosed.run();
    }
